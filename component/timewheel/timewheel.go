...
func New(tick time.Duration, wheelSize int64) *TimeWheel {
	...
	delayQueue := NewDelayQueue(int(wheelSize))
	return newTimeWheel(tickMs, wheelSize, time.Now().UnixMilli(), delayQueue)
}

func newTimeWheel(tickMs int64, wheelSize int64, startMs int64, delayQueue *DelayQueue) *TimeWheel {
	// 初始化任务桶
...
func (tw *TimeWheel) Start() {
	...
	tw.waitGroup.Wrap(func() {
		tw.delayQueue.Poll(tw.stop, func() int64 {
	...
	tw.waitGroup.Wrap(func() {
		for {
			select {
			case item := <-tw.delayQueue.Channel():
				b := item.(*bucket)
				// 调整指针
...
func (t *Timer) Reset(d time.Duration) bool {
	now := time.Now()
	if t.expiration <= now.UnixMilli() { // 说明任务已执行完成
		return false
	}
	t.expiration = now.Add(d).UnixMilli()
	return true
}
